...
namespace Matchplay.Shared.Infrastructure
{
    ...
    public class UpdateRunner : MonoBehaviour
    {
        ...
        /// <summary>
        /// Subscribe in order to have onUpdate called approximately every period seconds (or every frame, if periodSeconds <= 0).
        /// Don't assume that onUpdate will be called in any particular order compared to other subscribers.
        /// </summary>
        public void Subscribe(Action<float> onUpdate, float periodSeconds)
        {
            if (onUpdate == null)
            {
                return;
            ...
            m_subscribers.Add(new Subscriber(onUpdate, periodSeconds));
        }
